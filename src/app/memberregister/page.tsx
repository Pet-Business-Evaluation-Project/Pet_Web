  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      if (storedUser) {
        ...
        if (
          user.classification !== "관리자" &&
          user.classification !== "심사원"
        ) {
          setHasAccess(false);
          setError(
            "접근 권한이 없습니다. 관리자 또는 심사원만 이용 가능합니다."
          );
        }
      } else {
        // 로그인하지 않은 경우에도 접근 거부
        setHasAccess(false);
        setError(
          "로그인 후 이용 가능합니다. 관리자 또는 심사원만 접근할 수 있습니다."
        );
      }
    }
  }, []);
  ...
  const fetchMySigns = async () => {
    ...
    try {
      if (currentUser.classification === "관리자") {
        ...
        const reviewerRes = await axiosInstance.post("/user/reviwerinfo", {
          userId: currentUser.id,
        });
        const myReviewerId: number = reviewerRes.data.reviewerId;
        console.log("✅ 내 심사원 ID:", myReviewerId);

        const allSignsRes = await axiosInstance.get<SignStart[]>(
          "/signstart/all"
        );
        const mySigns = allSignsRes.data.filter(
          (sign) => sign.reviewerId === myReviewerId
        );
        const uniqueMap = new Map<number, SignStart>();
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                ...
                >
                  {signtypeOptions
                    .filter((opt) =>
                      opt === "미정" ? !currentSign.signtype : true
                    )
                    .map((opt) => (
              ...
              <div className="space-y-2">
                ...
                <select
                  value={
                    currentSign.membergrade
                      ? membergradeMap[currentSign.membergrade]
                      : "심사 중"
                  }
                  ...
                  disabled={!isAdmin}
                >
                  {Object.values(membergradeMap).map((opt) => (
                    <option key={opt} value={opt}>
                      {opt}
                    </option>
                  ))}
              ...
              <div className="space-y-2">
                ...
                <select
                  ...
                  onChange={(e) =>
                    handleChange("reviewcomplete", e.target.value)
                  }
                  className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all bg-gray-50"
                >
                  {reviewcompleteOptions.map((opt) => (
                    <option key={opt} value={opt}>
                      {opt}
                    </option>
                  ))}
                  ...
                  filteredSigns.map((sign) => (
                    ...
                    >
                      ...
                      <td className="px-8 py-5 text-gray-700">
                        {sign.membergrade
                          ? membergradeMap[sign.membergrade]
                          : "심사 중"}
                      </td>
                      ...
                      <td className="px-8 py-5">
                        <span
                          className={`inline-flex px-3 py-1 rounded-full text-sm font-medium ${
                            sign.reviewcomplete === "심사완료"
                              ? "bg-green-100 text-green-700"
                              : sign.reviewcomplete === "진행중"
