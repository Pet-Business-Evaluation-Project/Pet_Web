...
const ReviewerDetailModal: React.FC<ReviewerDetailModalProps> = ({
  reviewer,
  onClose,
}) => {
  if (!reviewer) return null;

  const maskedSsn = reviewer.ssn
    ? `${reviewer.ssn.substring(0, 8)}******`
    : "미입력";
      ...
      <div className="bg-white rounded-xl shadow-2xl max-w-xl w-full border border-gray-100">
        ...
        <div className="p-8 space-y-6">
          ...
          <DetailItem
            title="은행/계좌"
            content={
              reviewer.bankname && reviewer.account
                ? `${reviewer.bankname} / ${reviewer.account}`
                : "미입력"
            }
          />
          <DetailItem
            title="전문 분야"
            content={reviewer.expertises || "미입력"}
          />
...
export default function ReviewerDashboard() {
  ...
  const exportToCSV = (data: Reviewer[]) => {
    ...
    const csvContent = data
      .map((r) => {
        // 직책 검증 추가
        const validGrade = ["심사원보", "심사위원", "수석심사위원"].includes(
          r.reviewergrade
        )
          ? r.reviewergrade
          : "심사원보";
    ...
    link.setAttribute(
      "download",
      `심사원_목록_${new Date().toISOString().slice(0, 10)}.csv`
    );
        ...
        if (res.ok) {
          ...
          const data: Reviewer[] = rawData.map((r) => {
            ...
            if (
              ...
              console.warn(
                `Invalid grade for ${r.name} (${r.loginID}): "${gradeRaw}"`
              );
