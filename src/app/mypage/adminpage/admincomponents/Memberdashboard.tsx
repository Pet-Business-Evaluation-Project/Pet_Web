...
export default function MemberDashboard() {
  ...
  const fetchCompanies = async () => {
    setLoading(true);
    try {
      const res = await fetch("https://www.kcci.co.kr/back/mypage/admin/members", {
        method: "POST",
        headers: {
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch("https://www.kcci.co.kr/back/mypage/admin/members/update", {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      <div className="bg-white rounded-2xl shadow-lg p-6">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="text-sm text-gray-600 bg-gray-100 px-4 py-2 rounded-lg">
            총 <span className="font-bold text-blue-600">{companies.length}</span>개 기업
          </div>
        </div>
              ...
              <table className="w-full border-collapse text-sm">
                <thead className="bg-gradient-to-r from-blue-50 to-indigo-50">
                  <tr className="text-left border-b-2 border-blue-200">
                    ...
                    <th className="py-3 px-4 font-semibold text-gray-700">아이디</th>
                    <th className="py-3 px-4 font-semibold text-gray-700">대표자명</th>
                    <th className="py-3 px-4 font-semibold text-gray-700">사업자번호</th>
                    <th className="py-3 px-4 font-semibold text-gray-700">연락처</th>
                    <th className="py-3 px-4 font-semibold text-gray-700">이메일</th>
                    <th className="py-3 px-4 font-semibold text-gray-700">가입일</th>
                    <th className="py-3 px-4 font-semibold text-gray-700 text-center">관리</th>
                  </tr>
                </thead>
                <tbody>
                  {companies.length === 0 ? (
                    <tr>
                      <td colSpan={8} className="text-center py-12 text-gray-500">
                        <FaBuilding className="mx-auto mb-3 text-gray-300" size={48} />
                        <p>등록된 기업이 없습니다.</p>
                      </td>
                    </tr>
                  ) : (
                    companies.map((company, index) => (
                      ...
                      >
                        ...
                        <td className="py-3 px-4 font-medium text-blue-600">{company.loginID}</td>
                        <td className="py-3 px-4">{company.name}</td>
                        <td className="py-3 px-4 text-gray-600">{formatBusinessNumber(company.sno)}</td>
                        <td className="py-3 px-4 text-gray-600">{company.phnum}</td>
                        <td className="py-3 px-4 text-gray-600">
                          <div className="max-w-[200px] truncate" title={company.email}>
                            {company.email || "-"}
                          </div>
                        </td>
                        <td className="py-3 px-4 text-gray-600">{formatDate(company.createdAt)}</td>
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <InfoField label="사업자번호" value={formatBusinessNumber(selectedCompany.sno)} />
                  <InfoField label="연락처" value={selectedCompany.phnum} />
                  <InfoField label="주소" value={selectedCompany.address} />
                  <InfoField label="담당자" value={selectedCompany.referralID || "-"} />
                  <InfoField label="가입일" value={formatDate(selectedCompany.createdAt)} />
                </div>
              </div>
                  ...
                  {isEditMode ? (
                    <>
                      ...
                      <EditField 
                        ...
                        onChange={(value) => setEditData({...editData, companycls: value})}
                        placeholder="예: 제조업, IT서비스업"
                      />
                      <EditField 
                        label="회사소개" 
                        value={editData.introduction}
                        onChange={(value) => setEditData({...editData, introduction: value})}
                        multiline
                    ...
                    <>
                      <InfoField label="이메일" value={selectedCompany.email || "-"} />
                      <InfoField label="사업분류" value={selectedCompany.companycls || "-"} />
                      <InfoField 
                        label="회사소개" 
